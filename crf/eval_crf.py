...
def eval_crf(args):
    ...
    post = ['_%d'%(i+1) for i in range(args.save_iter)]
    ...
    for i in range(len(post)):
        for idx, img_id in tqdm(enumerate(img_ids)):
            ...
            pred_path = os.path.join(args.crf_sem_seg_out_dir, 'pseudo_mask_%s%s.png' % (img_id, post[i]))
            pred = Image.open(pred_path)
            pred = np.array(pred, dtype=np.uint8)  # shape = [h, w]
            mIOU_list[i].add_batch(pred, gt)
