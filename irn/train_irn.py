...
from metadata.dataset import VOC12AffinityDataset, VOC12ImageDataset
from util import pyutils, torchutils, indexing
import importlib

def run_train_irn(args):
    ...
    train_dataset = VOC12AffinityDataset(img_name_list_path=args.datalist,
    ...
    model = torch.nn.DataParallel(model).cuda() # to(args.device)
    model.train()

    avg_meter = pyutils.AverageMeter('loss1', 'loss2', 'loss3', 'loss4')
    timer = pyutils.Timer()
    ...
    infer_dataset = VOC12ImageDataset(args.datalist,
                                        voc12_root=args.data_root,
                                        crop_size=args.irn_crop_size,
